...
class ContaMaiores {
    public static void main(String[] args) {
        ...
        System.out.println(t1.contaMaiores(tgt));
...
class BST {
    ...
    */
    ...
    private String preOrder(Node current) {
        ...
        if (current.left != null) {
            if (current.right != null) {
                ...
                return current.value + ", " + preOrder(current.left);
            }
        } else if (current.right != null) {
            if (current.left == null) {
                return current.value + ", " + preOrder(current.right);
            }
        }
    ...
    public int contaMaiores(int target) {
        return contaMaiores(target, this.root);
    }

    public int contaMaiores(int target, Node current) {
        if (current == null) {
            return 0;
        }

        if (current.value > target) {
            return 1 + contaMaiores(target, current.left) + contaMaiores(target, current.right);
        }

        return contaMaiores(target, current.right);
    }
}
