...
class RoundTree {
    public static void main(String[] args) {
        ...
        System.out.println(t1.proximo(tgt));
...
class BST {
    ...
    */
    ...
    public int proximo(int target) {
        Node current = this.root;
        
        int closest = this.root.value;
        int min = Math.abs(this.root.value - target);
        while (current != null) {
            if (Math.abs(current.value - target) < min) {
                closest = current.value;
                min = Math.abs(current.value - target);
            }
